...
if __name__ == "__main__":
    # Part 1 and 2
    with open('../inputs/day11.txt', 'r') as inputFile:
    ...
    while True:
        ...
        if opcode == 1:
            ...
            inputBuffer = grid[currentPos] if currentPos in grid.keys() else 0
            ...
            outputBuffer.append(firstParam)
            base += 2
            if len(outputBuffer) == 2:
                # Set Grid Colour
                grid.pop(currentPos, None)
                grid[currentPos] = outputBuffer[0]
                # Change Direction
                xChange = list(grid.keys())[-1][0] - currentPos[0]
                yChange = list(grid.keys())[-1][1] - currentPos[1]
                if xChange == 0:
                    currentPos = (currentPos[0], currentPos[1] + (-1 if outputBuffer[1] == 0 else 1))
                else:
                    currentPos = (currentPos[0] + (-1 if outputBuffer[1] == 0 else 1), currentPos[1])
                outputBuffer = []
        elif opcode == 5:
            if firstParam != 0:
                base = secondParam
            ...
            print("Higher than 844: {}".format(len(set(grid.keys()))))
            break
        else:
